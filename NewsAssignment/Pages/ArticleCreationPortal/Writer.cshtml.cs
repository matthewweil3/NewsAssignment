...
namespace NewsAssignment.Pages.ArticleCreationPortal
{
    ...
    public class WriterModel : PageModel
    {
                            ...
                            string Image, string Color, string Content, string Icon, string Creator)
        {
            ...
            Article = new Article { title = Title, description = Description, image_url = Image, content = Content, category = Category, pubDate = DateTime.UtcNow.ToString(), creator = User.Identity.Name, language = "en", link = null, status=Article.State.Authored};
